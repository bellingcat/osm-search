<template>
  <v-container>
    <v-card style="width: 100%" variant="flat">
      <v-card-title class="my-2">
        <v-row class="ma-2">
          Search area
          <v-spacer></v-spacer>
          <v-select v-model="mode" :items="mapModes" variant="outlined" density="compact" hide-details="true"
            label="Map mode" class="mx-2">
          </v-select>
          <v-text-field variant="outlined" density="compact" hide-details="true" label="Find location"
            prepend-inner-icon="mdi-magnify" @keypress.enter="searchLocation" v-model="locationSearch"></v-text-field>
        </v-row>
      </v-card-title>
      <v-card-text>
        <l-map v-model:zoom="zoom" :center="center" style="width: 100%; height: 80vh" ref="map"
          :noBlockingAnimations="true" @ready="updateBbox" @update:bounds="updateBbox">
          <l-tile-layer :url="url" />
          <map-marker v-for="result in store.searchResults" :key="'marker' + result.index" :result="result" />
